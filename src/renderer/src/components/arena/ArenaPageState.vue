...
<template>
  <div class="arena-page-state">
    <Transition name="arena-state" mode="out-in">
      <div v-if="holdLoading" key="loading" class="arena-page-state__skeleton">
        <slot name="skeleton">
          <ArenaPageSkeleton :variant="skeleton" :label="loadingLabel" />
        </slot>
      </div>
      <div v-else-if="error" key="error" class="arena-page-state__panel arena-page-state__error">
        <p>{{ error }}</p>
