...
<template>
  <div class="page-view-host">
    <Transition name="page-route" mode="out-in">
      <div v-if="pageError" key="error" class="page app-empty-state page-view-host__panel">
      ...
      <div
        v-else-if="pageLoading || !pageComponent"
        key="loading"
        ...
        aria-label="加载页面"
      >
        <ArenaPageSkeleton :variant="skeletonVariant" label="加载页面" />
      </div>
